...
export function CruisePortDayAIBuilder({tripId,cruiseDay,canManage}:{tripId:string;cruiseDay:any;canManage:boolean}){
  ...
  return <section className="panel cruise-ai-builder">
    <div className="section-title-row"><div><div className="eyebrow">Smart Planning</div><h2>✨ Build My Port Day with AI</h2><div className="muted">Create a safe draft around your interests. Nothing is saved until you approve it.</div></div><button className="secondary" onClick={()=>setOpen(!open)}>{open?"Close Builder":"Plan Suggestions"}</button></div>
    {open?<div className="cruise-ai-body"><div className="ai-step"><span>1</span><div><strong>Choose your interests</strong><small>Select everything you would enjoy ashore.</small></div></div><div className="interest-chips">{options.map(x=><button className={interests.includes(x)?"active":""} onClick={()=>toggle(x)} key={x}>{x}</button>)}</div><div className="ai-step"><span>2</span><div><strong>Choose your pace</strong><small>Travel Crew will keep the safe return window protected.</small></div></div><div className="field"><label>Pace</label><select id="cruise-ai-pace" defaultValue="Moderate"><option>Relaxed</option><option>Moderate</option><option>Busy</option></select></div><div className="ai-step"><span>3</span><div><strong>Build the draft</strong><small>Review every suggestion before adding it.</small></div></div><button className="primary ai-build-cta" onClick={build} disabled={busy}>{busy?"Building…":"✨ Build Suggested Port Day"}</button>
