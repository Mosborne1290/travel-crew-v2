...
export function TripJournal({tripId,userId,initialEntries,photos}:{tripId:string;userId:string;initialEntries:Entry[];photos:Photo[]}){
 ...
 async function save(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);const {error}=await supabase.from("journal_entries").upsert({trip_id:tripId,user_id:userId,entry_date:date,title:String(f.get("title")||"")||null,notes:String(f.get("notes")||"")||null,highlight:String(f.get("highlight")||"")||null,favourite_moment:String(f.get("favourite_moment")||"")||null,updated_at:new Date().toISOString()},{onConflict:"trip_id,user_id,entry_date"});if(error)setMessage(error.message);else{await refresh();setMessage("Journal saved.");}}
